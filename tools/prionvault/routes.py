...
def api_list_articles():
    ...
    if collection_id:
        ...
        if c and c["kind"] == "smart":
            ...
            has_pdf      = merged.get("has_pdf")
            has_doi      = merged.get("has_doi")
            has_pmid     = merged.get("has_pmid")
            pdf_source_filter = merged.get("source")
    ...
    try:
        return _list_articles_with_recovery(
            ...
            sort, page, page_size,
            is_jc=is_jc,
            search_fields=search_fields, q_mode=q_mode,
...
def _list_articles_impl(s, q, year_min, year_max, journal,
    ...
    pv_select = ", ".join(
        ...
        ["pdf_md5", "pdf_pages", "pdf_is_scan",
         ...
         # boolean flags below.
         "source",
         "abstract_unavailable", "pdf_oa_status", "oa_author_requested_at",
    ...
    def _row_to_dict(r):
        ...
        out = {
            ...
            "oa_author_requested": bool(d.get("oa_author_requested_at")),
...
def api_article_detail(aid):
    ...
    try:
        ...
        optional = [
            ...
            "abstract_unavailable", "pubmed_unavailable", "oa_author_requested_at",
    ...
    try:
        ...
        return jsonify([
            {"id": r.id, "name": r.name, "color": r.color, "count": r.n_articles,
             ...
             "is_mine": bool(_viewer_id()) and r.created_by is not None and str(r.created_by) == str(_viewer_id())}
            for r in rows
        ])
    finally:
        s.close()
...
def api_create_tag():
    ...
    Pass kind="smart" + rules to create a smart tag instead of a plain
    one — its article_tag_link rows are then generated by
    services.smart_tags.sync_tag() (run once immediately below) rather
    than assigned by hand."""
    from .services import smart_tags
    ...
    try:
        ...
        t = models.ArticleTag(name=name, color=color, kind=kind, rules=rules,
                              created_by=vid)
        s.add(t)
        s.commit()
        out = t.to_dict()
        ...
        if kind == "smart":
            try:
                out["sync"] = smart_tags.sync_tag(out["id"], vid)
            except Exception:
                logger.exception("initial smart tag sync failed for %s", out["id"])
...
@prionvault_bp.route("/api/tags/<int:tag_id>/rules", methods=["PUT"])
@login_required
def api_update_tag_rules(tag_id):
    """Set (or change) a tag's smart-rule criteria, converting it to
    kind='smart' if it wasn't already, then re-syncs immediately.
    Allowed for the tag's creator or an admin — unlike renaming (which
    affects the shared dictionary everyone sees), a smart tag's rules
    only ever affect that creator's own auto-generated assignments."""
    from .services import smart_tags
    ...
    try:
        ...
        if not is_admin and str(t.created_by) != str(vid):
            return jsonify({"error": "forbidden",
                            "detail": "Solo quien creó esta tag (o un Admin) puede editar sus reglas."}), 403
        creator = t.created_by or vid
        t.kind = "smart"
        t.rules = rules
        s.commit()
        s.close()
        sync = smart_tags.sync_tag(tag_id, creator)
        ...
        s.rollback()
        logger.exception("api_update_tag_rules failed")
        return jsonify({"error": "internal_error", "detail": str(e)[:300]}), 500
    finally:
        s.close()


@prionvault_bp.route("/api/tags/<int:tag_id>/sync", methods=["POST"])
@login_required
def api_sync_tag(tag_id):
    """Manually re-run a smart tag's rules right now, instead of
    ...
    try:
        ...
        if not t:
            return jsonify({"error": "not_found"}), 404
        if t.kind != "smart":
            return jsonify({"error": "not_a_smart_tag"}), 400
        is_admin = (session.get("role") == "admin")
        if not is_admin and str(t.created_by) != str(vid):
    ...
    try:
        ...
        logger.exception("api_sync_tag failed")
        return jsonify({"error": "internal_error", "detail": str(e)[:300]}), 500
...
@login_required
def api_jc_file_office_pdf(fid):
    """PDF rendering of a Word/Excel/PowerPoint JC file (see
    services.jc.get_or_convert_pdf) — converted via LibreOffice on
    first view, cached in Dropbox afterwards. Fed to the same PDF.js
    viewer template used for real PDFs."""
    from .services import jc as _jc
    pdf_bytes = _jc.get_or_convert_pdf(fid)
    if not pdf_bytes:
        return jsonify({
            "error": "conversion_failed",
            "detail": "No se pudo generar una vista previa de este documento.",
        }), 502
    return Response(pdf_bytes, mimetype="application/pdf", headers={
