    ...
    with db.engine.begin() as conn:
        ...
        if existing_id:
            ...
            placeholders = ", ".join(f":{c}" for c in fields.keys()) + \
                           ", 'extracted', :added_by, NOW(), NOW()"
            sql = (f"INSERT INTO articles ({', '.join(cols)}) "
                   f"VALUES ({placeholders}) RETURNING id")
            params = dict(fields)
            params["added_by"] = (str(fields.get("added_by"))
                                  if fields.get("added_by") else None)
            # added_by is mapped to added_by_id in the DB.
            params.pop("added_by", None)
            params["added_by"] = (str(kw.get("added_by"))
                                  if kw.get("added_by") else None)
