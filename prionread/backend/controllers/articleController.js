...
/**
 * Resolves article metadata from CrossRef (by DOI) and/or PubMed (by PMID).
 * When CrossRef returns no abstract, automatically searches PubMed by DOI
 * to find the PMID and fetch the abstract from there.
 */
async function resolveExternalMetadata(doi, pubmed_id) {
  if (doi) {
    ...
    if (!meta.abstract) {
      // CrossRef had no abstract — try PubMed (by provided PMID or DOI lookup)
